...
template_root="${repo_root}/.github/openwrt/luci-app-fileuni"
stage_root="$(mktemp -d)"
...
cat > "${nfpm_config}" <<EOF
name: luci-app-fileuni
arch: ${package_arch}
platform: linux
version: ${version}
section: luci
priority: optional
maintainer: FileUni Team <contact@fileuni.com>
description: FileUni OpenWrt LuCI app and procd service wrapper
homepage: https://github.com/FileUni
license: MIT
depends:
  - luci-base
contents:
...
package_path="$(find "${output_dir}" -maxdepth 1 -type f -name 'luci-app-fileuni*.ipk' | head -1 || true)"
if [ -z "${package_path}" ] || [ ! -f "${package_path}" ]; then
  printf 'Failed to locate generated luci-app package in %s\n' "${output_dir}" >&2
  exit 1
fi
